...
def mamba_chunk_scan(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, dt_softplus=False, init_state=None):
    """
    Argument:
        x: (batch, seqlen, nheads, headdim)
        dt: (batch, seqlen, nheads)
    ...
    if init_state is not None:
        init_state = torch.zeros_like(states[:, :1])
    states = torch.cat([init_states, states], dim=1)  # (bchpn)
