...
def load_wkv5_cuda_kernel(head_size):
    ...
    rwkv5_cuda_kernel.head_size = head_size  # type: ignore
...
class WKV_5(torch.autograd.Function):
    ...
    def forward(ctx, B, T, C, H, r, k, v, w, u, s):
        with torch.no_grad():
            ...
            rwkv5_cuda_kernel.forward(B, T, C, H, r, k, v, eew, u, y, s)  # type: ignore
            return y, s
    ...
    def backward(ctx, gy):
        with torch.no_grad():
            ...
            rwkv5_cuda_kernel.backward(B, T, C, H, r, k, v, eew, ew, u, gy, gr, gk, gv, gw, gu)  # type: ignore
...
def rwkv_linear_attention(
    ...
    if rwkv5_cuda_kernel is None or no_cuda or one_token:
        ...
        out, state = WKV_5.apply(B, T, H * S, H, receptance, key, value, time_decay, time_first, state)  # type: ignore
        out = out.reshape(B * T, H * S)
        out = F.group_norm(out, num_groups=H, weight=lxw, bias=lxb).reshape(B, T, H * S)
        out = out.to(dtype=hidden.dtype) * gate
        out = out @ ow
...
class RwkvBlock(nn.Module):
    def __init__(self, config, layer_id):
        super().__init__()
        print(f'[Block.__init__] {layer_id=}')
        self.config = config
        self.layer_id = layer_id
