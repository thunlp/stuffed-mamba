...
class Block(nn.Module):
    ...
    def forward(
        ...
        **mixer_kwargs,
        ...
        hidden_states, conv_state, ssm_state = self.mixer(
            hidden_states,
            states=states,
        ...
        if self.mlp is not None:
            if not self.fused_add_norm:
                ...
                if self.residual_in_fp32:
                    residual = residual.to(torch.float32)  # type: ignore
            else:
                hidden_states, residual = layer_norm_fn(
                    hidden_states,
                    self.norm2.weight,
...
class Mamba2(nn.Module):
    def __init__(
        ...
        self.do_norm_states = False
        self.state_norm_threshold = None
        self.B_mult = None
        self.A_mult = None
        self.dA_mult = None
        self.dt_mult = None
    ...
    @torch.no_grad()
    def set_B_clip(self, val: float):
        self.B_clip = val

    @torch.no_grad()
    def set_dA_mult(self, val: float):
        self.dA_mult = val

    @torch.no_grad()
    def set_A_mult(self, val: float):
        self.A_mult = val

    @torch.no_grad()
    def set_A_offset(self, val: float):
        self.A_offset = val

    @torch.no_grad()
    def set_B_mult(self, val: float):
        self.B_mult = val

    @torch.no_grad()
    def set_dt_mult(self, val: float):
        self.dt_mult = val

    @torch.no_grad()
    def set_dt_offset(self, val: float):
        self.dt_bias.data = self.dt_bias.data * torch.tensor(val, dtype=self.dt_bias.dtype)

    def use_state_norm(self, norm_thresholds: list[float]):
        '''
        Args:
        - norm_thresholds: one float per head. 
        '''
        self.do_norm_states = True
        # (H)
        self.state_norm_threshold = torch.tensor(norm_thresholds, dtype=torch.float, device='cuda')
    ...
    def forward(
        ...
        if inference_params is not None:
            ...
            if self.do_norm_states:
                ys = []
                chunk_size = 128
                cur_states = (conv_state, ssm_state)
                for i in range(0, seqlen, chunk_size):
                    this_u = u[:, i:i+chunk_size]
                    out, new_conv_state, new_ssm_state = self.forward(this_u, states=cur_states)
                    cur_norm = torch.norm(new_ssm_state, dim=(-1, -2))
                    thresholds = self.state_norm_threshold.unsqueeze(0)  # (b, h)
                    scale = torch.where(cur_norm > thresholds, cur_norm / thresholds, 1)  # (b, h)
                    scale = rearrange(scale, 'b h -> b h 1 1')
                    new_ssm_state = new_ssm_state * scale
                    ys.append(out)
                    cur_states = (new_conv_state, new_ssm_state)
        ...
        if False and self.use_mem_eff_path and inference_params is None:
            ...
            if self.B_clip is not None:
                B = torch.clamp(B, max=self.B_clip)
            ...
            if self.A_offset is not None:
                A = A + self.A_offset

            if self.dA_mult is not None:
                dt = dt + self.dA_mult * (A ** (-1))
    ...
    def step(
        ...
        conv_state: Tensor,
        ssm_state: Tensor,
        norm_state: bool = False,
        state_norm_threshold: float = 1.0,
    ):
        """
        Args:
            hidden_states
            conv_state
            ssm_state
            norm_state (bool): Whether to use inference-time state normalization, states are normalized
        """
        ...
        **mixer_kwargs,
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if grad_ckpt is None or grad_ckpt == "none":
                ...
                hidden_states, residual, conv_state, ssm_state = checkpoint(
                    layer,
                    hidden_states,
                    residual,
                    layer_state,
...
class Mamba2ForCausalLM(nn.Module, GenerationMixin):
    ...
    @torch.no_grad()
    def start_state_norm(
        self,
        input_ids: Tensor,
        threshold_multiplier: float = 2.0,
    ):
        hidden_states, last_states = self.backbone(input_ids)
        ...
        for i in range(self.config.n_layer):
            # (...), (B, H, P, N)
            ssm_state = last_states[i][1][0]  # (H, P, N)
            layer_norms = []
            for head_state in ssm_state:
                # print(head_state.shape)
                head_norm = torch.norm(head_state)
                # breakpoint()
                layer_norms.append(head_norm)
            norms.append(layer_norms)

        for i, layer in enumerate(self.backbone.layers):
            layer.mixer.use_state_norm(norms[i])
    ...
    def forward(
        ...
        **mixer_kwargs,
        ...
        hidden_states, last_states = self.backbone(
            input_ids,
            inference_params=inference_params,
            states=states,
            grad_ckpt=grad_ckpt,
        ...
        CausalLMOutput = namedtuple("CausalLMOutput", ["logits", "states"])
        new_states = MambaState(last_states)
        return CausalLMOutput(logits=lm_logits, states=new_states)
